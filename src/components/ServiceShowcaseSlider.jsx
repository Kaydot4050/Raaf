...
export default function ServiceShowcaseSlider({ items = [] }) {
  ...
  const updateTransforms = useCallback(() => {
    ...
    slideRefs.current.forEach((el, i) => {
      ...
      next[i] = { scale: scaleFromOffset(offset, slideSpan) };
    });
  ...
  const go = useCallback(
    (delta) => {
      ...
      if (!count) return;
      const next = Math.max(0, Math.min(count - 1, getCenterIndex() + delta));
      scrollToIndex(next);
    },
  ...
  useEffect(() => {
    if (!len) return;
    scrollToIndex(0);
  }, [len, width, height, gap, scrollToIndex, items]);
    ...
    <div className="relative w-full">
              ...
              return (
                ...
                >
                  {slide.image ? (
                    ...
                    <div className="h-full w-full flex flex-col items-center justify-center bg-gradient-to-br from-forest/25 via-forest/10 to-cream px-4">
                      {Icon ? <Icon className="w-12 h-12 sm:w-16 sm:h-16 text-forest/70" strokeWidth={1.25} /> : null}
                      <span className="mt-3 text-[10px] font-bold uppercase tracking-[0.15em] text-forest/80">{slide.role}</span>
                    </div>
                  )}
                  {slide.image ? (
                    <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-charcoal/70 via-charcoal/20 to-transparent p-4 pt-12">
                      <span className="text-[10px] font-bold uppercase tracking-[0.12em] text-white/80">{slide.role}</span>
                    </div>
                  ) : null}
                </button>
              );
      ...
      {mouseControl && (
        <p className="mt-3 text-center text-xs text-text-muted hidden sm:block">
          Point left or right over the gallery to move one service at a time
        </p>
      )}

      <div key={active} className="mt-6 sm:mt-8 text-center px-4">
        <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-forest mb-2">{activeItem.role}</p>
        <h4 className="font-display text-xl sm:text-2xl font-bold text-charcoal">{activeItem.name}</h4>
        <p className="mt-2 text-sm sm:text-base text-text-muted max-w-lg mx-auto leading-relaxed">{activeItem.short}</p>
      </div>
    </div>
  );
}
